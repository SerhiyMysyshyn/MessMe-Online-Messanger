...
class FriendsViewModel @Inject constructor(
    ...
    fun getFriendRequests(needFetch: Boolean = false) {
        getFriendRequestsUseCase(needFetch) {
    ...
    fun deleteFriend(friendEntity: FriendEntity, homeActivity: HomeActivity, friendsViewModel: FriendsViewModel) {
        deleteFriendUseCase(friendEntity) {
            if(it.isRight){
                friendsViewModel.getFriends()
                homeActivity.replaceFragment(FriendsFragment(homeActivity))
            }

            it.either(::handleFailure, ::handleDeleteFriend)
        }
    }
